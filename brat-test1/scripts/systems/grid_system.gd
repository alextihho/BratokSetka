# grid_system.gd (ИСПРАВЛЕНО - КЛИКИ ПО ВСЕЙ СЕТКЕ)
extends CanvasLayer
...
# ✅ НОВОЕ: Offset карты (если карта сдвинута)
var map_offset: Vector2 = Vector2(0, 0)  # Будет вычислен автоматически
...
func _ready():
	layer = 5  # ✅ КРИТИЧНО! Выше карты (0), но ниже UI (50)
...
func get_square_at_position(pos: Vector2) -> String:
	# ✅ ИСПРАВЛЕНО: Блокируем отрицательные координаты и выход за границы
	print("🎯 GRID CHECK: pos=" + str(pos))
	
	# ✅ Блокируем отрицательные координаты (клики за левым краем экрана)
	if pos.x < 0:
		print("   ❌ X отрицательный (x=%d)" % pos.x)
		return ""
	
	# ✅ Блокируем клики за правым краем
	if pos.x >= 720:
		print("   ❌ X за границей экрана (x=%d >= 720)" % pos.x)
		return ""
	
	# ✅ Проверяем верхнюю границу
	if pos.y < 120:
		print("   ❌ Верхняя панель (y=%d < 120)" % pos.y)
		return ""
	
	# ✅ Проверяем нижнюю границу
	if pos.y >= 1180:
		print("   ❌ Нижняя панель (y=%d >= 1180)" % pos.y)
		return ""
	...
	print("   Сетка: x=%d, y=%d (adjusted_y=%d)" % [grid_x, grid_y, adjusted_y])
	
	# ✅ Проверка границ сетки
	if grid_x < 0 or grid_x >= grid_width:
		print("   ❌ X вне границ! (0-%d)" % (grid_width - 1))
		return ""
	
	if grid_y < 0 or grid_y >= grid_height:
		print("   ❌ Y вне границ! (0-%d)" % (grid_height - 1))
		return ""
	
	var square_id = "%d_%d" % [grid_x, grid_y]
	
	# ✅ Проверяем существование квадрата
	if not grid_squares.has(square_id):
		print("   ❌ Квадрат не существует: " + square_id)
		return ""
	
	print("   ✅ Квадрат найден: " + square_id)
	return square_id
