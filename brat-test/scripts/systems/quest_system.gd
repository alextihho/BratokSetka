...
# Статистика игрока для квестов
var player_stats_data = {
	"total_earned": 0,  # Всего заработано денег
	"battles_won": 0,   # Побед в боях
	"items_bought": []  # Купленные предметы
}
...
func update_quest(quest: Dictionary, data: Dictionary):
	match quest["type"]:
		"collect":
			# Проверяем текущий баланс игрока
			if data.has("balance"):
		...
		"earn":
			# Проверяем сколько всего заработано
			if data.has("earned"):
...
func complete_quest(quest_id: String):
	...
	# Сохраняем информацию о завершённом квесте
...
func show_quests_menu(main_node: Node):
	...
	completed_header_bg.name = "CompletedHeader"
	...
	completed_label.name = "CompletedToggle"
	...
	# Контейнер для завершённых квестов
	...
	close_btn.position = Vector2(20, 1160)
