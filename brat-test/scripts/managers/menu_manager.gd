# menu_manager.gd (ИСПРАВЛЕН - LAYER 150 ДЛЯ МЕНЮ)
extends Node
...
func show_main_menu(main_node: Node):
	...
	menu_layer.layer = 150  # ✅ КРИТИЧНО! Выше сетки (5) и UI (50)
	main_node.add_child(menu_layer)
	
	# ✅ Полупрозрачный overlay (блокирует клики на сетку)
...
func show_stats_window(main_node: Node):
	...
	stats_popup.layer = 150  # ✅ Выше сетки
