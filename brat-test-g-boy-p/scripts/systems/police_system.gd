...
# Проверка вызова полиции после преступления (на месте преступления)
func check_police_after_crime(main_node: Node) -> bool:
	if ua_level >= 100:
		print("🚨 УА ДОСТИГ 100! ПОЛИЦИЯ ВЫЗВАНА НА МЕСТО ПРЕСТУПЛЕНИЯ!")
		show_surrender_menu(main_node)
		return true
	return false
