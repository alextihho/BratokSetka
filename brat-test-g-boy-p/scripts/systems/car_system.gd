...
func buy_car(main_node: Node, player_data: Dictionary, car_id: String, car: Dictionary, selection_menu: CanvasLayer):
	...
	player_data["current_driver"] = -1  # ✅ По умолчанию игрок водитель
	player_data["in_car"] = true  # ✅ НОВОЕ: Автоматически садимся в машину после покупки

	main_node.show_message("🚗 Поздравляем с покупкой: %s!\n🟢 Вы сели в машину (водитель: Вы)" % car["name"])
