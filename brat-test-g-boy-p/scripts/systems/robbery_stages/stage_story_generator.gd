...
# ✅ НОВОЕ: Генерация с учетом типа здания
static func generate_entry_story_with_building(method: String, is_success: bool, stat_used: String, building_type: String) -> String:
	# Специальные истории для разных типов зданий
	var building_stories = {
		"ЛАРЁК_success_stealth": [
			"Подходишь к ларьку со стороны задней двери. Старый замок поддается с третьей попытки. Внутри пусто — продавец отлучился покурить. Везет.",
			"Окно в подсобке приоткрыто. Аккуратно раздвигаешь створки и проскальзываешь внутрь. Запах табака и просроченных чипсов."
		],
		"КВАРТИРА_success_stealth": [
			"Подъезд пуст, камер нет. Отмычка скользит в замочную скважину — щелчок за щелчком. Дверь открывается бесшумно.",
			"Соседи громко ругаются за стеной — идеальное прикрытие. Замок старый, открывается за секунды. Внутри тихо."
		],
		"ГАРАЖ_success_stealth": [
			"Ворота не закрыты на ключ — только на щеколду. Тихо отодвигаешь засов и проскальзываешь внутрь. Пахнет маслом и бензином.",
			"Боковая дверь слабо держится на петлях. Надавливаешь — поддается. Внутрь проникаешь без шума."
		],
		"МАГАЗИН_success_force": [
			"Удар ногой в старую дверь — та вылетает с треском. Сигнализация молчит. Внутри пусто, свет горит.",
			"Витрина хлипкая. Бита врезается в стекло — осколки разлетаются. Прыгаешь внутрь, хрустя обувью по битому стеклу."
		],
		"СКЛАД_success_force": [
			"Замок на воротах старый, ржавый. Лом вставляешь в щель — резкий рывок, и цепь рвется. Ворота со скрипом открываются.",
			"Удар кувалдой по навесному замку — металл трещит и разлетается. Ворота открыты."
		],
		"ОФИС_success_tech": [
			"Электронный замок взламывается за минуту. Коды подобраны, зеленый свет — можно входить. Офис пуст, уборщица ушла.",
			"Камеры отключаешь с телефона — взлом их сети оказался детской забавой. Дверь открывается бесшумно."
		]
	}

	# Пробуем найти специфичную историю
	var key = building_type + "_success_" + method if is_success else building_type + "_fail_" + method
	if building_stories.has(key):
		var stories = building_stories[key]
		return stories[randi() % stories.size()]

	# Если нет специфичной — используем общую
	return generate_entry_story(method, is_success, stat_used)
